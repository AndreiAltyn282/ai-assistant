...
// === НАСТРОЙКИ ===
const API_URL = 'http://localhost:8000/ask'; // API бэкенда
...
function activate(context) {
    ...
    let explainCommand = vscode.commands.registerCommand('ai-assistant.explain', async function () {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('❌ Нет активного редактора!');
            return;
        ...
        if (!selectedText) {
            vscode.window.showErrorMessage('❌ Выделите код!');
            return;
        }

        await askAI(selectedText, 'Объясни этот код построчно');
    });

    context.subscriptions.push(askCommand, explainCommand);
}
...
async function askAI(code, question) {
    try {
        ...
        // Показываем ответ в отдельном окне
        ...
        <head>
            ...
            <style>
                ...
                .code { background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 8px; white-space: pre-wrap; font-family: 'Consolas', monospace; }
                .answer { background: #2d2d2d; padding: 15px; border-radius: 8px; white-space: pre-wrap; margin-top: 10px; }
...
function deactivate() {
    console.log('❌ AI Assistant деактивирован');
}
